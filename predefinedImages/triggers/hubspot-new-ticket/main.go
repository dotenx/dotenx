// image: hojjat12/hubspot-new-ticket:lambda
package main
...
func HandleLambdaEvent(event Event) (Response, error) {
	...
	tickets, err := listTickets(accessToken, selectedUnix)
	if err != nil {
		fmt.Println(err)
		return resp, err
	}
	if len(tickets) == 0 {
		fmt.Println("no new ticket in last", passedSeconds, "seconds")
	...
	returnValue[triggerName] = tickets
...
func listTickets(accessToken string, afterUnix int64) (tickets []map[string]interface{}, err error) {
	nextLink := "https://api.hubapi.com/crm/v3/objects/tickets?limit=100"
	...
	for i := 0; i < MAX_PAGE_NUMBER; i++ {
		...
		if err != nil || statusCode != http.StatusOK {
			fmt.Println("HubSpot response (list tickets):", string(out))
		...
		for _, ticket := range resp.Results {
			createdTime, err := time.Parse(time.RFC3339, ticket["createdAt"].(string))
			if err != nil {
				fmt.Println(err)
				return nil, err
			}
