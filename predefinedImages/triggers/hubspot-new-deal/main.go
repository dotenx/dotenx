// image: hojjat12/hubspot-new-deal:lambda
package main
...
func HandleLambdaEvent(event Event) (Response, error) {
	...
	deals, err := listDeals(accessToken, selectedUnix)
	if err != nil {
		fmt.Println(err)
		return resp, err
	}
	if len(deals) == 0 {
		fmt.Println("no new deal in last", passedSeconds, "seconds")
...
func listDeals(accessToken string, afterUnix int64) (deals []map[string]interface{}, err error) {
	nextLink := "https://api.hubapi.com/crm/v3/objects/deals?limit=100"
	for {
		headers := []Header{
			{
				Key:   "Content-Type",
		...
		if err != nil || statusCode != http.StatusOK {
			fmt.Println("HubSpot response (list deals):", string(out))
		...
		for _, deal := range resp.Results {
			createdTime, err := time.Parse(time.RFC3339, deal["createdAt"].(string))
			if err != nil {
				fmt.Println(err)
				return nil, err
			}
