// image: hojjat12/stripe-create-product:lambda
package main
...
func HandleLambdaEvent(event Event) (Response, error) {
	...
	product, err := createProduct(sc, name, currency, int64(unitAmountInt), recurringInterval, int64(recurringIntervalCountInt))
	if err != nil {
		fmt.Println(err.Error())
		resp.Successfull = false
	}
	...
	if resp.Successfull {
		...
		resp.Status = "failed"
		fmt.Println("Product can't be created successfully")
	}
	return resp, nil
}
