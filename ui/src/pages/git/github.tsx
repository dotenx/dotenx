import { Button, Group, Loader, Modal, Select, Text, TextInput } from '@mantine/core'
...
const GithubIntegration = ({ resetProvider }: { resetProvider: () => void }) => {
	...
	return (
		<div className="mx-auto bg-rose-100  rounded-md p-5 w-2/3">
			...
			>
				<ExportRepoModal gitId={selectedValue || ''} repoName={selectedRepo} />
			</Modal>
			<Modal
			...
			>
				<ImportRepoModal gitId={selectedValue || ''} repoName={selectedRepo} />
			</Modal>
		</div>
	)
}

const ExportRepoModal = ({ gitId, repoName }: { gitId: string; repoName: string }) => {
...
const ImportRepoModal = ({ gitId, repoName }: { gitId: string; repoName: string }) => {
	const { projectName = '' } = useParams()
