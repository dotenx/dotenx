...
func TestDeleteUserInfo(t *testing.T) {
	...
	deleteRowQuery := "DELETE FROM user_info WHERE account_id = $1"
	mock.ExpectExec(regexp.QuoteMeta(deleteRowQuery)).WithArgs(userInfo.AccountId).WillReturnResult(sqlmock.NewResult(0, 1))

	err = UserManagementStore.DeleteUserInfo(db, userInfo.AccountId)
	t.Log(err)
