...
func (ps *pipelineStore) ListOffBoardingPipelines(context context.Context, accountId string) (pipelines []models.WorkspacePipelineSummary, err error) {
	// In the future we can use different statements based on the db.Driver as per DB Engine

	switch ps.db.Driver {
	case db.Postgres:
		conn := ps.db.Connection
		err = conn.Select(&pipelines, listOffBoardingPipelines, accountId)
		if err != nil {
			return
		}
	}
...
WHERE p.account_id = $1 AND t.trigger_type = 'OffBoarding';
`
