...
func (p *pipelineStore) GetById(context context.Context, id int) (pipeline models.PipelineSummery, err error) {
	...
	case db.Postgres:
		...
		err = conn.QueryRow(select_pipeline_by_id, id).Scan(&pipeline.PipelineDetailes.Id, &pipeline.Endpoint, &pipeline.IsActive, &pipeline.IsTemplate, &pipeline.IsInteraction, &ug, &pipeline.ProjectName)
		if err != nil {
		...
		tasks := []models.Task{}
		var rows *sql.Rows
		rows, err = conn.Query(select_tasks_by_pipeline_id, id)
		if err != nil {
			log.Println("error", err.Error())
			return
		}
