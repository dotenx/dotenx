...
var getProjectByTag = `
Select id, name, account_id, description, tag, has_database from projects
WHERE tag = $1
`
...
func (store *projectStore) GetProjectByTag(ctx context.Context, tag string) (models.Project, error) {
	...
	for rows.Next() {
		if err := rows.Scan(&project.Id, &project.Name, &project.AccountId, &project.Description, &project.Tag, &project.HasDatabase); err != nil {
			return models.Project{}, err
		}
	}
	return project, nil
