...
func (ds *objectstoreStore) ListFiles(ctx context.Context, accountId, projectTag string) ([]models.Objectstore, error) {
	...
	listFiles := `
SELECT key, account_id, tpaccount_id, size, project_tag, access
FROM   object_store
...
`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&objectstore.Key, &objectstore.AccountId, &objectstore.TpAccountId, &objectstore.Size, &objectstore.ProjectTag, &objectstore.Access); err != nil {
			return nil, err
		}
