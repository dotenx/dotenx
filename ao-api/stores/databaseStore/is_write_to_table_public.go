...
func (ds *databaseStore) IsWriteToTablePublic(ctx context.Context, projectTag string, tableName string) (bool, error) {
	...
	if val, ok := commentMap["isWritePublic"].(bool); ok {
		return val, nil
	} else {
		logrus.Error("isWritePublic field not found")
