...
func (store *marketplaceStore) ListItemsByCategory(ctx context.Context, category string) ([]models.MarketplaceItem, error) {
	listItems := `
	SELECT * FROM marketplace_items WHERE category = $1 and enabled = true
	`
	var stmt string
	...
	rows, err := store.db.Connection.Queryx(stmt, category)
	if err != nil {
