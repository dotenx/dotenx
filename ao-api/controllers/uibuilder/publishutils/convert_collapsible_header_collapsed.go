...
const collapsibleHeaderCollapsedTemplate = `{{if .RepeatFrom.Iterator}}<template {{if .RepeatFrom.Name}}x-for="({{.RepeatFrom.Iterator}}, index) in {{renderRepeatFromName .RepeatFrom.Name}}"{{end}}>{{end}}<div id="{{if .ElementId}}{{.ElementId}}{{else}}{{.Id}}{{end}}" class="{{range .ClassNames}}{{.}} {{end}}"   {{renderEvents .Events}} {{if .RepeatFrom.Name}}:key="index"{{end}} x-show='toggle ? active == $el.parentElement.id : isOpen[$el.parentElement.id]' @click='toggle ? active = -1 : isOpen[$el.parentElement.id] = false'>{{.RenderedChildren}}</div>{{if .RepeatFrom.Iterator}}</template>{{end}}`
...
func convertCollapsibleHeaderCollapsed(component map[string]interface{}, styleStore *StyleStore, functionStore *FunctionStore) (string, error) {
	...
	id := collapsibleHeaderCollapsed.Id
	if collapsibleHeaderCollapsed.ElementId != "" {
		id = collapsibleHeaderCollapsed.ElementId
	}
	styleStore.AddStyle(id, collapsibleHeaderCollapsed.Data.Style.Desktop, collapsibleHeaderCollapsed.Data.Style.Tablet, collapsibleHeaderCollapsed.Data.Style.Mobile)
	styleStore.AddCustomStyle(id, collapsibleHeaderCollapsed.Data.CustomStyle.Desktop, collapsibleHeaderCollapsed.Data.CustomStyle.Tablet, collapsibleHeaderCollapsed.Data.CustomStyle.Mobile)
