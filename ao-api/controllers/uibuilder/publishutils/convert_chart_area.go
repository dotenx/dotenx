...
const areaChartEffectTemplate = `
Alpine.effect(() => {
	const data = Alpine.store("{{.StoreName}}")?.{{.StoreName}};
	if (data) {
		areaChart().renderChart({ data, xlabel: "{{.Xlabel}}", ylabel: "{{.Ylabel}}", chartId: "{{if .ElementId}}{{.ElementId}}{{else}}{{.Id}}{{end}}", title: "{{.Title}}",
...
func convertChartArea(component map[string]interface{}, styleStore *StyleStore, functionStore *FunctionStore) (string, error) {
	...
	tmpl, err := template.New("chartArea").Funcs(funcMap).Parse(chartTemplate)
	if err != nil {
		fmt.Println(err)
		return "", err
	}
	...
	effectTmpl, err := template.New("areaChartEffect").Parse(areaChartEffectTemplate)
	if err != nil {
		fmt.Println(err)
		return "", err
	}
	...
	functionStore.AddChart("Area", effectOut.String())
