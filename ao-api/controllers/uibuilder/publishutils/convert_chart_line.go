...
func convertChartLine(component map[string]interface{}, styleStore *StyleStore, functionStore *FunctionStore) (string, error) {
	...
	tmpl, err := template.New("chartLine").Funcs(funcMap).Parse(chartTemplate)
	if err != nil {
		fmt.Println(err)
		return "", err
	}
	...
	effectTmpl, err := template.New("lineChartEffect").Parse(lineChartEffectTemplate)
	if err != nil {
		fmt.Println(err)
		return "", err
	}
	...
	functionStore.AddChart("Line", effectOut.String())
