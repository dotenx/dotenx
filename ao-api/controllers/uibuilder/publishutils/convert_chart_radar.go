...
const radarChartEffectTemplate = `
Alpine.effect(() => {
	const data = Alpine.store("{{.StoreName}}")?.{{.StoreName}};
	if (data) {
		radarChart().renderChart({ data, xlabel: "{{.Xlabel}}", ylabel: "{{.Ylabel}}", chartId: "{{if .ElementId}}{{.ElementId}}{{else}}{{.Id}}{{end}}", title: "{{.Title}}",
...
func convertChartRadar(component map[string]interface{}, styleStore *StyleStore, functionStore *FunctionStore) (string, error) {
	...
	effectTmpl, err := template.New("radarChartEffect").Parse(radarChartEffectTemplate)
	if err != nil {
		fmt.Println(err)
		return "", err
	}
	...
	functionStore.AddChart("Radar", effectOut.String())
