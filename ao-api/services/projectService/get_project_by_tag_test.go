...
func TestGetProjectByTag(t *testing.T) {
	...
	rows := sqlmock.NewRows([]string{"id", "name", "account_id", "description", "tag", "has_database", "type", "theme"}).
		AddRow(expectedProject.Id, expectedProject.Name, expectedProject.AccountId, expectedProject.Description, expectedProject.Tag, expectedProject.HasDatabase, expectedProject.Type, expectedProject.Theme)
	...
	mock.ExpectQuery(regexp.QuoteMeta(selectQuery)).
		WithArgs(expectedProject.Tag).
		WillReturnRows(rows)

	// call GetProjectByTag function to get the project by tag
	project, err := ProjectService.GetProjectByTag(expectedProject.Tag)
	if err != nil {
